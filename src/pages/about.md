---
title: "About"
layout: layouts/base.njk
permalink: "/about/"
---

<div style="text-align: center; margin-bottom: 4rem;">
  <h1 style="font-size: 2.6em; line-height: 1.3; margin-bottom: 1.5rem;">A space for making, learning, and sharing the process.</h1>
  <p style="font-size: 1.2em; color: #57534e; font-style: italic;">Projects I'm building. Skills I'm learning. Systems that actually work.</p>
</div>

## What This Is

Creative Mind Habits is where I document what I'm making and learning. It's part build log, part learning archive, part idea collection.

I make things, learn new skills, and share what works. No polish required, just progress.

<div style="height: 2rem;"></div>

## What You'll Find

- **Build logs** for projects I'm working on (web apps, classroom tools, this blog)
- **Learning documentation** (touch typing, Korean, video editing, project management)
- **Systems and workflows** that make work easier
- **Book notes** and what I'm taking from them
- **Tools and templates** I've built along the way

<div style="height: 2rem;"></div>

## If You're Here, You Probably…

- Like making things and learning new skills
- Appreciate seeing the process, not just the result
- Build systems that support your work
- Enjoy learning in public

<div style="height: 2rem;"></div>

<div style="background: #f5f5f4; border: 1px solid #e7e5e4; border-radius: 6px; padding: 2rem 1rem; margin: 3rem 0; text-align: center;">
  <h3 style="margin-bottom: 1.5rem; font-size: 1.3em;">Operating Principles</h3>
  <p style="line-height: 1.8; margin-bottom: 0; font-size: 0.95em;">
    Progress over polish<br>
    Share from the middle<br>
    Build in public<br>
    Learn by doing<br>
    Make useful things
  </p>
</div>

<div style="text-align: center; padding: 2.5rem 0; margin: 3rem 0; border-top: 1px solid #e7e5e4; border-bottom: 1px solid #e7e5e4;">
  <p style="font-size: 1.1em; margin-bottom: 1.5rem;">Explore</p>
  <p>
    <a href="/tools/" style="display: inline-block; padding: 14px 28px; background: #1c1917; color: white; text-decoration: none; border-radius: 6px; font-weight: 600; margin: 0.5rem;">Tools & Templates →</a>
    <a href="/creator-systems/" style="display: inline-block; padding: 14px 28px; background: white; color: #292524; text-decoration: none; border: 1px solid #d6d3d1; border-radius: 6px; font-weight: 600; margin: 0.5rem;">Creator Systems</a>
  </p>
</div>

<div style="background: #f5f5f4; border-left: 3px solid #57534e; padding: 2rem; margin: 3rem 0; border-radius: 4px;">
  <p style="font-weight: 600; font-size: 1.1em; margin-bottom: 0.75rem;">About</p>
  <p>I'm <strong>Em</strong> — I teach, build web tools, and learn new things constantly. Creative Mind Habits is my home base for sharing all of it.</p>
  <p style="margin-bottom: 0;">This is a long-term project documenting what I make and what I learn along the way.</p>
</div>

<p style="text-align: center; color: #78716c; font-style: italic; margin-top: 3rem;">Thanks for being here.</p>